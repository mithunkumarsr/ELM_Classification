function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,TY] = elm_kernel(TrainingData_File, TestingData_File, Elm_Type, Regularization_coefficient, Kernel_type, Kernel_para)
...
elseif strcmp(kernel_type,'sig_kernel')
    if nargin<4,
        omega = Xtrain*Xtrain';
        omega = tanh(omega./kernel_pars(1));
    else
       omega = Xtrain*Xt';
        omega = tanh(omega./kernel_pars(1));
    end
...
elseif strcmp(kernel_type,'poly_kernel')
    if nargin<4,
        omega = (Xtrain*Xtrain'+kernel_pars(1)).^2;
    else
        omega = (Xtrain*Xt'+kernel_pars(1)).^2;
    end
